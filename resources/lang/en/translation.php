<?php 
/*-------------------------------------------------
// Automatically generated language file 
// on 22/12/2022 09:50:07; do not modify manually, 
// as it will be overwritten by the next generation 
// of the language file; changes must be made 
// using the system translation options
-------------------------------------------------*/
return [
"3_min_ago" => "3 min ago",
"Topbar_Light" => "Topbar Light",
"TUI_Calendar" => "TUI Calendar",
"Add_Product" => "Add Product",
"Advanced_Plugins" => " Advanced Plugins",
"Advanced_Table" => " Advanced Tables",
"Alert_Email" => "Alert Email",
"Alerts" => "Alerts",
"Apex_Charts" => "Apexcharts",
"Applications" => "Applications",
"Apps" => "Apps",
"As_a_skeptical_Cambridge_friend_of_mine_occidental" => "As a skeptical Cambridge friend of mine occidental.",
"Authentication" => "Authentication",
"Basic_Action" => "Basic Action",
"Basic_Elements" => "Basic Elements",
"Billing_Email" => "Billing Email",
"Blog_Details" => "Blog Details",
"Blog_Grid" => "Blog Grid",
"Blog_List" => "Blog List",
"Blog" => "Blog",
"Blog" => "Blog",
"Bootstrap_Basic_Tables" => "Bootstrap Basic Tables",
"Bootstrap_Basic" => "Bootstrap Basic",
"Basic_Tables" => "Basic Tables",
"Bootstrap" => "Bootstrap",
"Boxed_Width" => "Boxed Width",
"Boxed_Width" => "Boxed Width",
"Boxicons" => "Boxicons",
"Boxicons" => "Boxicons",
"Buttons" => "Buttons",
"Buy_Sell" => "Buy/Sell",
"Calendars" => "Calendar",
"Cards" => "Cards",
"Carousel" => "Carousel",
"Cart" => "Cart",
"Chartjs_Charts" => "Chartjs",
"Chartjs" => "Chartjs",
"Charts" => "Charts",
"Chat" => "Chat",
"Checkout" => "Checkout",
"Colored_Header" => "Colored Header",
"Colored_Sidebar" => "Colored Sidebar",
"Colors" => "Colors",
"Coming_Soon" => "Coming Soon",
"Compact_Sidebar" => "Compact Sidebar",
"Components" => "Components",
"Confirm_Mail" => "Confirm Mail",
"Contacts" => "Contacts",
"Create_New" => "Create New",
"Create_Task" => "Create Task",
"Crypto" => "Crypto",
"Crypto" => "Crypto",
"Customers" => "Customers",
"Dashboards" => "Dashboard",
"Data_Tables" => "DataTables",
"Default" => "Default",
"Dripicons" => "Dripicons",
"Dropdowns" => "Dropdowns",
"E_Charts" => "Echarts",
"Echarts" => "Echarts",
"Ecommerce" => "Ecommerce",
"Edit_Details" => "Edit Details",
"Editable_Table" => "Editable Tables",
"Editable" => "Editable",
"Editors" => "Editors",
"Elements" => "Elements",
"Email_verification" => "Email Verification",
"Email" => "Email",
"Error_404" => "Error 404",
"Error_500" => "Error 500",
"Exchange" => "Exchange",
"Extended" => "Extended",
"Extra_Pages" => "Extra Pages",
"FAQs" => "FAQs",
"Feather" => "Feather",
"File_Manager" => "File Manager",
"File_Upload" => "File Upload",
"Flot_Charts" => "Flot Charts",
"Font_awesome" => "Font Awesome",
"Form_Advanced" => "Form Advanced",
"Form_Editors" => "Form Editors",
"Form_Elements" => "Form Elements",
"Form_File_Upload" => "Form File Upload",
"Form_Layouts" => "Form Layouts",
"Form_Mask" => "Form Mask",
"Form_Repeater" => "Form Repeater",
"Form_Validation" => "Form Validation",
"Form_Wizard" => "Form Wizard",
"Form_Xeditable" => "Form Xeditable",
"Forms_Advanced_Plugins" => "Form Advanced Plugins",
"Forms" => "Forms",
"Full_Calendar" => "Full Calendar",
"General" => "General",
"Google_Maps" => "Google Maps",
"Google" => "Google",
"Google" => "Google",
"Grid" => "Grid",
"Horizontal_layout" => "Horizontal layout",
"Horizontal" => "Horizontal",
"ICO_Landing" => "ICO Landing",
"Icon_Sidebar" => "Icon Sidebar",
"Icons" => "Icons",
"Icons" => "Icons",
"If_several_languages_coalesce_the_grammar" => "If several languages coalesce the grammar",
"Images" => "Images",
"Inbox" => "Inbox",
"Invoice_Detail" => "Invoice Detail",
"Invoice_List" => "Invoice List",
"Invoices" => "Invoices",
"It_will_seem_like_simplified_English" => "It will seem like simplified English.",
"James_Lemire" => "James Lemire",
"Jquery_Knob_Charts" => "Jquery Knob",
"Jquery_Knob" => "Jquery Knob",
"Kanban_Board" => "Kanban Board",
"KYC_Application" => "KYC Application",
"Layouts" => "Layouts",
"Leaflet_Maps" => "Leaflet Maps",
"Leaflet" => "Leaflet ",
"Leaflet" => "Leaflet",
"Lending" => "Lending",
"Level_1.1" => "Level 1.1",
"Level_1.2" => "Level 1.2",
"Level_2.1" => "Level 2.1",
"Level_2.2" => "Level 2.2",
"Light_Sidebar" => "Light Sidebar",
"Lightbox" => "Lightbox",
"Lock_screen" => "Lock screen",
"Lock_Screen" => "Lock Screen",
"Login" => "Login",
"Logout" => "Log Out",
"Maintenance" => "Maintenance",
"Maps" => "Maps",
"Mask" => "Mask",
"Material_Design" => "Material Design",
"Material_Design" => "Material Design",
"Mega_Menu" => "Mega Menu",
"Menu" => "Menu",
"Modals" => "Modals",
"Multi_Level" => "Multi Level",
"My_Wallet" => "My Wallet",
"New" => "New",
"Notifications" => "Notifications",
"Offcanvas" => "Offcanvas",
"Orders" => "Orders",
"Orders" => "Orders",
"Pages" => "Pages",
"Placeholders" => "Placeholders",
"Preloader" => "Preloader",
"Preloader" => "Preloader",
"Pricing" => "Pricing",
"Product_Detail" => "Product Detail",
"Products" => "Products",
"Profile" => "Profile",
"Profile" => "Profile",
"Progress_Bars" => "Progress Bars",
"Project_Overview" => "Project Overview",
"Projects_Grid" => "Projects Grid",
"Projects_List" => "Projects List",
"Projects" => "Projects",
"Range_Slider" => "Range Slider",
"Rating" => "Rating",
"Read_Email" => "Read Email",
"Recover_Password" => "Recover Password",
"Register" => "Register",
"Responsive_Table" => "Responsive Tables",
"Responsive" => "Responsive",
"Saas" => "Saas",
"Salena_Layfield" => "Salena Layfield",
"Scrollable" => "Scrollable",
"Scrollable" => "Scrollable",
"Search" => "Search...",
"Session_Timeout" => "Session Timeout",
"Settings" => "Settings",
"Shops" => "Shops",
"Sparkline_Charts" => "Sparkline Charts",
"Sparkline" => "Sparkline",
"Starter_Page" => "Starter Page",
"Sweet_Alert" => "SweetAlert",
"Tables" => "Tables",
"Tabs_&_Accordions" => "Tabs & Accordions",
"Task_List" => "Task List",
"Tasks" => "Tasks",
"Templates" => "Templates",
"Timeline" => "Timeline",
"Toast_UI_Charts" => "Toast UI Charts",
"Toasts" => "Toasts",
"Two_step_verification" => "Two Step Verification",
"Typography" => "Typography",
"UI_Components" => "UI Components",
"UI_Elements" => "UI Elements",
"User_Grid" => "User Grid",
"User_List" => "User List",
"Utility" => "Utility",
"Validation" => "Validation",
"Vector_Maps" => "Vector Maps",
"Vector" => "Vector ",
"Video" => "Video",
"View_All" => "View All",
"View_More" => "View More..",
"Wallet" => "Wallet",
"Wizard" => "Wizard",
"Your_item_is_shipped" => "Your item is shipped",
"Your_order_is_placed" => "Your order is placed",
"Line_Charts" => "Line Charts",
"Line" => "Line",
"Area" => "Area",
"Area_Charts" => "Area Charts",
"Column_Chart" => "Column Chart",
"Column" => "Column ",
"Mixed" => "Mixed",
"Mixed_Chart" => "Mixed Charts",
"Timeline" => "Timeline",
"Timeline_Charts" => "Timeline Charts",
"Candlestick" => "Candlestick",
"Candlestick_Charts" => "Candlestick Charts",
"Boxplot" => "Boxplot",
"Boxplot_Charts" => "Boxplot Charts",
"Bubble" => "Bubble",
"Bubble_Charts" => "Bubble Charts",
"Scatter" => "Scatter",
"Scatter_Charts" => "Scatter Charts",
"Heatmap" => "Heatmap",
"Heatmap_Charts" => "Heatmap Charts",
"Treemap_Charts" => "Treemap Charts",
"Treemap" => "Treemap ",
"Pie_Charts" => "Pie Charts",
"Pie" => "Pie ",
"Radialbar_Charts" => "Radiabar Charts",
"Radialbar" => "Radialbar ",
"Radar_Charts" => "Radar Charts",
"Radar" => "Radar ",
"Polararea_Charts" => "Polararea Charts",
"Polararea" => "Polararea ",
"Bar" => "Bar",
"Bar_Charts" => "Bar Charts",
"Eva_Icons" => "Eva Icons",
"Client_menu" => "Clients (new)",
"Client_created" => "Client has been created",
"Client_updated" => "Client has been updated",
"Client_deleted" => "Client has been deleted",
"Client_admin" => "Client Administration",
"Client_fantasy" => "Fantasy name",
"Client_name" => "Client name",
"Client_address" => "Address",
"Client_add" => "Create client",
"Client_edit" => "Update client",
"Actions" => "Actions",
"Actions_add" => "Add",
"Actions_edit" => "Edit",
"Actions_update" => "Update",
"Actions_delete" => "Delete",
"Confirm_delete_title" => "Are you shure you want to delete this record?",
"Confirm_delete_text" => "This action cant be reverted",
"Confirm_delete_button" => "Delete",
"Confirm_cancel_button" => "Cancel",
"Show_confirm_deleted_title" => "Record deleted",
"Multilingual" => "Multilingual",
"Languages" => "Languages",
"Translations" => "Translations",
"Language_admin" => "Language Administration",
"Language_full" => "Language",
"Language_abbr" => "Abbreviation",
"Language_act" => "Active",
"Language_add" => "Create language",
"Language_edit" => "Edit language",
"Language_created" => "Language has been created",
"Language_updated" => "Language has been updated",
"Language_deleted" => "Language has been deleted",
"Translation_expression" => "Expression",
"Translation_traduction" => "Traduction",
"Translation_admin" => "Traductions Administration",
"Translation_edit" => "Edit traduction",
"Translation_generate" => "Generate translations",
"1_hours_ago" => "1 hours ago",
"Show_Confirm_deleted_text" => "The record has been successfully deleted; remember to generate the language file for the changes to take effect.",
"Translation_file_generated_title" => "Translation file generated",
"Translation_file_generated_text" => "Translation file has been successfully generated",
"TransFile_admin" => "Translation Files Administration",
"TransFile_created" => "Translation file has been created",
"TransFile_updated" => "Translation file has been updated",
"TransFile_deleted" => "Create Translate file",
"TransFile" => "Files",
"Translation_add" => "Add translation",
"TransFile_file" => "File",
"TransFile_description" => "Description",
"TransFile_add" => "Add translation file",
"Translation_file" => "File",
"Translation_Import" => "Import from excel file",
"Translation_Export" => "Export from excel file",
"print" => "Print",
"Select_import" => "Select the csv, xls or xlsx file to import",
"TransFile_upload" => "Upload translation file",
];